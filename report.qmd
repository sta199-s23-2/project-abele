...
police_shootings <- police_shootings %>%
  ...
  mutate(race_num = case_when(
         ...
         threat_level_num = as.numeric(factor(`Factors.Threat-Level`)))

# fit linear regression model
model <- linear_reg() %>%
  set_engine("lm") %>%
  fit(`threat_level_num` ~ race_num + Person.Age + region_num, data = police_shootings) %>%
  tidy()
...
threat_level_num: {1 if attack; 2 if other; 3 if undetermined; 4 if unknown}
